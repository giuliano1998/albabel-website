...
export function PresenciaChina() {
  const dots = [
    { city: 'China', top: '30.5%', left: '79.2%' },
    { city: 'Venezuela', top: '48.5%', left: '32.8%' },
    { city: 'Paraguay', top: '65.5%', left: '33.5%' },
    { city: 'Argentina', top: '72%', left: '33.5%' },
  ]
  return (
    <section className="section dark">
      <div className="container grid g-2" style={{ alignItems: 'center', gap: 50 }}>
        ...
        <Reveal delay={.15}>
          <div className="map-wrap card" style={{ padding: 0, aspectRatio: '4378 / 2435', position: 'relative', overflow: 'hidden' }}>
            <img src="/assets/images/world-map.svg" alt="Presencia operativa de Albabel en China, Argentina, Paraguay y Venezuela"
                 style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            {dots.map((d) => (
              <span key={d.city} className="map-pin" style={{ top: d.top, left: d.left }}>
                <span className="map-dot" title={d.city} />
                <span className="map-pin-label">{d.city}</span>
              </span>
            ))}
          </div>
        </Reveal>
...
export function Carrusel() {
  return (
    <section id="casos" className="section" style={{ background: 'var(--white)' }}>
      <div className="container">
        <Reveal>
          <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 44px' }}>
            ...
            <p className="lead" style={{ marginTop: 18 }}>
              Marcas reales que construyeron su operación con nosotros. Estas son sus historias, en primera persona.
            </p>
          </div>
        </Reveal>
...
export function VideoFinal() {
  return (
    <VideoFeature dark
      eyebrow="Una invitación" eyebrowVariant="light"
      title="Si llegaste hasta acá, ya sabés que no querés hacerlo solo"
      video={VIDEOS.giuliano} name="Giuliano" role="La invitación"
      ctaMsg="Hola Albabel, quiero empezar a importar desde China con ustedes.">
      Giuliano te cuenta, de persona a persona, por qué vale la pena dar el paso con un equipo que ya lo recorrió.
